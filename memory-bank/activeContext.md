...
- For glossary help: keep the term as a normal link to `glossary.md#...`, and add a separate GitBook footnote marker after the link, e.g. `[term](glossary.md#term) [^gl_term]`, with `[^gl_term]: ...` copied from `glossary.md` (tooltip shows on the footnote marker).
...
- Synced all glossary-term links to the pattern `[term](glossary.md#term) [^gl_term]` (footnote marker outside the link), with per-page `[^gl_...]` definitions synced from `glossary.md` using `scripts/sync_glossary_tooltips.py`.
