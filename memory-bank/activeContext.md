...
- For glossary help: keep the term as a normal link to `glossary.md#...`, and attach the definition as a GitBook footnote marker after the link with a space, e.g. `[term](glossary.md#term) [^gl_term]` (so the marker tooltip matches `glossary.md` and the link stays clickable).
